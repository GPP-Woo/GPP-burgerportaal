...
namespace ODBP.Features.Sitemap.SitemapInstances
{
    ...
    public class SitemapController(IOdrcClientFactory odrcClientFactory, BaseUri baseUri, ISimpleCache cache)
    {
            ...
            await foreach (var item in GetAllPages(odrcClient, $"{DocumentenQueryPath}&registratiedatumVanaf={vanaf:o}&registratiedatumTot={tot:o}", token))
            {
                ...
                model.Urls.Add(new()
                {
                    ...
                    Document = new()
                    {
                        DiWoo = new()
                        {
                            Creatiedatum = DateOnly.FromDateTime(document.Creatiedatum.DateTime).ToString("o"),
                            Publisher = publisher,
        ...
        {
            yield return new Documenthandeling { AtTime = document.Creatiedatum.ToString("o"), SoortHandeling = new() { Resource = "https://identifier.overheid.nl/tooi/def/thes/kern/c_641ecd76", Value = "vaststelling" } };

            if (document.Ontvangstdatum.HasValue)
            {
                yield return new Documenthandeling { AtTime = document.Ontvangstdatum.Value.ToString("o"), SoortHandeling = new() { Resource = "https://identifier.overheid.nl/tooi/def/thes/kern/c_dfcee535", Value = "ontvangst" } };
            }

            if (document.DatumOndertekening.HasValue)
            {
                yield return new Documenthandeling { AtTime = document.DatumOndertekening.Value.ToString("o"), SoortHandeling = new() { Resource = "https://identifier.overheid.nl/tooi/def/thes/kern/c_e1ec050e", Value = "ondertekening" } };
            }
        }
    }
    ...
    public class OdrcDocument
    {
        ...
        public required DateTimeOffset Creatiedatum { get; set; }
        public string? Omschrijving { get; set; }
        public OdrcKenmerk[]? Kenmerken { get; set; }

        public DateTimeOffset? Ontvangstdatum { get; set; }
        public DateTimeOffset? DatumOndertekening { get; set; }
    }
